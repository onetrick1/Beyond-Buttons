...
def eye_ratio():
    ...
    for a in range(100):
        ...
        if a == 15 or a == 45 or a == 75:
            cv2.putText(frame, 'BLINK NORMALLY', (int(frame_w/2), int(frame_h/2)), font, fontScale, color, thickness)  
    ...
    plt.plot(time1, right, label = "right eye") 
  
    plt.plot(time1, left, label = "left eye") 
    
    # naming the x axis 
    plt.xlabel("time in tenth of seconds") 
    # naming the y axis 
    plt.ylabel('ratio') 
    # giving a title to my graph 
    plt.title('Eye Ratio Calibration') 
...
def mouth_ratio():
    ...
    plt.plot(time1, mouth, label = "mouth") 
    ...
    plt.ylabel('ratio') 
    # giving a title to my graph 
    plt.title('Mouth Ratio Calibration') 
